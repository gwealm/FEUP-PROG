...
int main() {
    ...
    if (cin.good() || year < 0) {            // Invalid inputs
        cerr << "Invalid input for year\n";
        return -1;
    }

    displayFullCalendar(year);              // Displays a full-year calendar
 ...
 */
void displayMonthCalendar(int year, int month) {
    // Arrays containing months and the days of the week
    string months[12] = {"Jan", "Feb", "March", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
    string weekdays[7] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
    
    const int LIMIT = numberOfDays(month, year);
    const int AVG_WIDTH = 5;                                            // Width for the table
    ...
    for (int i = 0; i <= 6; i++) {
        if  (i == 6)
            ...
            cout << weekdays[i] << setw(AVG_WIDTH);
    }
    ...
    for (int day = 1; day <= LIMIT; day++) {
        ...
        if (day == 1) {                                     // First day of the month
            const int WIDTH = weekDay == 0 ? (6 * AVG_WIDTH + 1) : (weekDay - 1) * AVG_WIDTH + 1; // Calculating the initial width of the first day
            if (weekDay == 0)
                cout << setw(WIDTH) << day << endl;
            else
                cout << setw(WIDTH) << day << setw(AVG_WIDTH);
        } else {                                            // Every other day other than the 1st
            if (weekDay == 0)                               // If the day is saturday
                cout << day << endl;
            else if (day == 9)                              // If the day changes from 1 digit day to 2 digits (day 9 to 10)
                cout << day << setw(AVG_WIDTH+1); 
            else
                cout << day << setw(AVG_WIDTH);
        }
    }
    cout << endl;
}
...
/**
 * @brief       - Displays a full year calendar
 * 
 * @param       - int which represents the year 
 */
void displayFullCalendar(int year) {
